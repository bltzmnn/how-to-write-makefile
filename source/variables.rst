...
注意：不加括号的变量引用仅限于变量名为单字符的情况，如自动化变量。对于多字符的变量的引用必须加上
括号，否则make会将多字符变量名的首字母作为变量，如 ``$foo`` 表示变量 f 后面加上字符串“oo”。这和shell中变量的
引用不同，shell中变量可以是 ``$(foo)`` 或 ``$foo`` 。
...
是如果在变量中使用函数，那么，这种方式会让我们的make运行时非常慢，更糟糕的是，它会使得两
...
知道，变量可以定义在某个目标上。而我们可以给定一种“模式”，把变量定义在符合这种模式的所有目标上。
